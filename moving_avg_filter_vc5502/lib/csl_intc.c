...
{
	/* Clear IFR0 */
   IFR0 = CSL_INTC_IFR_RESET;

	/* Clear IFR1 */
	IFR1 = CSL_INTC_IFR_RESET;
}
...
)
{
    ...
    if (EventId > RCV2_EVENT)
    {
        ...
        bit = bit - XMT3_EVENT;
        old_flag  = (IER1&(0x0001<<bit))>>bit  ;
        IER1&=~(1<<bit);
    }
    /* EventId less than 15 */
    else
    {
        old_flag = (IER0&(0x0001<<bit))>>bit ;
        IER0&=~(1<<bit);
    }
    IRQ_globalRestore(old_intm);
...
{
	/* Clear IER0 */
	IER0 = CSL_CPU_IER0_RESETVAL;

	/* Clear IER1 */
	IER1 = CSL_CPU_IER1_RESETVAL;
}
